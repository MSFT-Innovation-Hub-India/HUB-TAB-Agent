...
prompt_ads = """
    ...
    **Instructions**
    ...
    - Add the above line items in the table for each Use case that needs to be discussed during the ADS. Choose from one or more topics to arrive at the full agenda, and fill in the details from input message in the context of each topic
    - Use the durations indicated below based on topics finalized
    ...
    - The output should be in the form of a well formed Markdown table format
    ...
    **Topic Type 1**: When the topic pertains to reviewing the architecture and Design of an existing Solution, and coming up with a futuristic architecture during the Engagement
    ...
    |----------------------|----------------------------------|----------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | ~ duration of 1 Hour | Architect Team of $Customer Name | Review of current Architecture of $SystemName <br><br> $SystemDescription              | During this session, we will review the current architecture. The following would be discussed:<br>- Overview of the Business functionalities served currently<br>- Overview of the Operational Requirements served currently<br>- Technical details of the current Solution Architecture<br>- Pain points in the current Architecture<br>- Limitations that need to be addressed in the to-be architecture<br>- Any new requirements to consider in the to-be architecture |
    | ~ duration of 3 Hours| $HubArchitect                    | Discuss to-be architecture of $SystemName <br><br> $SystemDescription                  | During this session, we will arrive at the to-be architecture. The following would be discussed:<br>- Potential Architecture & Design options for the System, including key Microsoft Platform Services required<br>- How the Business & Operational Requirements (BCDR, HA, performance, concurrency, Security, statutory requirements, etc.) can be met<br>- Migration considerations for the to-be architecture, as applicable<br>- A Technical Demo showcasing key components in the to-be architecture |
