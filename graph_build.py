...
class ToInputValidator(BaseModel):
    """Transfers work to a specialized assistant to validate the input from the meeting notes."""
...
class ToAgendaCreator(BaseModel):
    """Transfers work to a specialized assistant for draft agenda creation."""
...
class ToDocumentGenerator(BaseModel):
    """Transfers work to a specialized assistant to handle Microsoft Office Word document generation for the Agenda."""
...
def update_prompt_template_node(state: State):
    # Ensure the engagement type is available
    print("calling the update prompt template node")
    if "engagement_type" in state and state["engagement_type"]:
        engagement_type = state["engagement_type"]
        # Call set_prompt_template from agenda_selector.py
        ...
        # Optionally, persist the updated state in your cache if required:
...
def route_input_validation(state: State):
    ...
    safe_toolnames = [
        t.name if hasattr(t, "name") else t.__name__ for t in input_validator_tools
    ]
...
def route_agenda_creator(state: State):
    ...
    if did_cancel:
        return "leave_skill"
    # safe_toolnames = [
    #     t.name if hasattr(t, "name") else t.__name__ for t in agenda_generation_tools
    # ]
    # if all(tc["name"] in safe_toolnames for tc in tool_calls):
    #     return "agenda_generation_tools"
    return None
...
# After the input validation node
# builder.add_node("update_prompt_template", update_prompt_template_node)
# Add an edge from input_validation to update_prompt_template
# builder.add_edge("input_validation", "update_prompt_template")
# Then route to the primary assistant (or the next appropriate node)
# builder.add_edge("update_prompt_template", "primary_assistant")
