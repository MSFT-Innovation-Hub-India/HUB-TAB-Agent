...
class StateManagementBot(ActivityHandler):
    ...
    async def on_message_activity(self, turn_context: TurnContext):
        ...
        flag = set_blob_account_public_access(
            config.az_storage_account_name,
            config.az_subscription_id,
            config.az_storage_rg_name,
        )
        if not flag:
            self.logger.debug(
        ...
        if user_profile.name is None:
            ...
            try:
                ...
                messages = [
                    {
                        "role": "system",
                        "content": f'You are a city validation assistant. Based on the user input identify the match from the list of valid Innovation Hub location cities: {config.hub_cities}. Return a JSON response in the format {{"city": "matched_city_name"}} or {{"city": null}} if no match. Use your knowledge of the cities to validate the user input, even if the user provides synonyms for the city names.',
                    },
                    {
                        "role": "user",
                        "content": f"Is '{user_input}' a valid city in this list: {config.hub_cities}?",
                    },
                ]

                # Get the validation from Azure OpenAI
                response = client.chat.completions.create(
                    model=config.az_deployment_name,
                    messages=messages,
            ...
            if conversation_data.config["configurable"]["thread_id"] is None:
                ...
                client.beta.assistants.update(
                    config.az_assistant_id,
                    instructions=StateManagementBot.asst_sys_prompt,
                    tools=[{"type": "code_interpreter"}],
                    tool_resources={
                        "code_interpreter": {"file_ids": config.file_ids}
                    },
                    temperature=0.3,
                )
                self.logger.debug(
